...
def _parse_row(row) -> dict:
    try:
        ...
        fee_raw  = get('手續費')
        fee      = to_float(fee_raw) if fee_raw else 1
        tax_raw  = get('交易稅')
        tax      = to_float(tax_raw) if tax_raw else 0
...
def main():
    ...
    if not (8 <= today.day <= 12):
        print(f'  今天是 {today.day} 號，不在 8-12 號範圍內，結束')
        sys.exit(0)

    # 找 downloads 資料夾裡最新的 xls
    files = sorted(
