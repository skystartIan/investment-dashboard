...
（本檔 2026-07-06 由 __pycache__ 內的 .pyc 反編譯還原，行為與 v3.0 一致）
"""
import os
import json
import base64
...
def find_match(db: list, hashes: dict) -> str | None:
    ...
    for entry in db:
        if hash_distance(hashes['ph'], entry['ph']) < PHASH_THRESH \
                and hash_distance(hashes['ah'], entry['ah']) <= AHASH_THRESH \
                and hash_distance(hashes['dh'], entry['dh']) <= DHASH_THRESH:
            return entry['label']
    return None
    ...
    def do_GET(self):
        if self.path == '/config':
            self._send_json({
                ...
                'COMPANY_LNG':        os.environ.get('COMPANY_LNG', ''),
            })
        ...
        if self.path == '/captcha/lookup':
            try:
                ...
                for img in body.get('images', []):
                    ...
                    results[img['id']] = {'hashes': hashes, 'label': label}
...
if __name__ == '__main__':
    ...
    # 沒帶 --fg → 用 pythonw 無視窗重新啟動自己，然後結束（讓本視窗可關閉）
    if '--fg' not in sys.argv:
        pythonw = Path(r'C:\Python311-arm64\pythonw.exe')
        if pythonw.exists():
            subprocess.Popen(
    ...
    try:
        ...
        menu = pystray.Menu(
            pystray.MenuItem('元大 Config Server  ●  port 8765', None, enabled=False),
